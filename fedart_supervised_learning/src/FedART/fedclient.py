...
class FedARTClient(FedARTBase):
    ...
    def train(self):
        ...
        while True: #run until server end the FL process by closing connection
            print(f'\n===========Client {self.id} Round {self.round}=============\n')
            ...
            try:
                ...
                print(f'Client {self.id} exiting because Server must have ended FL process or some problem occured.')
            ...
            self.client_socket.shutdown(1) #1 means sent all, need to shutdown sending otherwise receive doesn't work
            ...
            new_model = pickle.loads(sdata) #model_data contains client id and model
            print(f'Client {self.id} received new server model')
            
            #update local model
            self.model.codes = copy.deepcopy(new_model.codes) #full replacement
